...
class _ConsumerCheckoutScreenState extends State<ConsumerCheckoutScreen> {
  ...
  Future<void> _processCheckout() async {
    ...
    try {
      ...
      final receiptUrl = await vendorService.uploadImage(
        _receiptImage!,
        'receipts',
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Reservation successful! Vendor is reviewing your payment.',
            ),
          ),
        );
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
            builder: (context) => const ConsumerMainScreen(initialIndex: 1),
          ),
          (route) => false,
        );
      }
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Total: RM ${price.toStringAsFixed(2)}',
                  ...
                  if (_vendorQrUrl != null && _vendorQrUrl!.isNotEmpty)
                      ...
                      child: Card(
                        child: Padding(
                          ...
                          child: _vendorQrUrl!.startsWith('data:image')
                              ? Image.memory(
                                  base64Decode(_vendorQrUrl!.split(',').last),
                                  height: 200,
                                  fit: BoxFit.contain,
                                )
                              : Image.network(
                    ...
                    const Card(
                      child: Padding(
                        ...
                        child: Center(
                          child: Text(
                            'Vendor has not set up DuitNow QR.\nPlease pay at counter.',
                          ),
                        ),
                      ),
                    ),
